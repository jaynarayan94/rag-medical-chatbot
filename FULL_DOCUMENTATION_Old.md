...
### 🐳 Step: Install Docker Engine if not present on your mac
```bash
docker --version
```
...
and also go inside  `custom_jenkins` using ```bash cd custom_jenkins``` before running point 3
...
Run the Jenkins container from the builded image(`docker build -t jenkins-dind .`) with the following command:
...
# Jenkins Python Setup and Restart Guide

## 9. Install Python and Set Up Jenkins

To install Python inside the Jenkins Docker container:

1. Enter the Jenkins container shell as the root user:
   ```bash
   docker exec -u root -it jenkins-dind bash
   ```

2. Update package lists inside the container:
   ```bash
   apt update -y
   ```
...
4. Verify the Python 3 installation:
   ```bash
   python3 --version
   ```

5. Create a symbolic link so that `python` points to `python3`:
   ```bash
   ln -s /usr/bin/python3 /usr/bin/python
   ```

6. Check the default python version:
   ```bash
   python --version
   ```

7. Install pip for Python 3:
   ```bash
...
- Open Jenkins UI in your browser at: `http://localhost:8080`
- Sign in using the initial admin password you retrieved from the logs.
...
1. Go to **SonarQube** -> **Create a Local Project**.
   - Enter a name for the project (e.g., `multi-ai-agent`).
   - Set the **Main Branch**.
   - Save the project.
...
# docker network connect ci-cd-network jenkins-dind
# docker network connect ci-cd-network sonarqube-dind
...
---
6. Restart **jenkins-dind** after the plugins installation:

```bash
docker restart jenkins-dind
```

7. Enter the Jenkins container shell as the root user:
```bash
docker exec -u root -it jenkins-dind bash
```
...
---
# the error is just because you already have a container named jenkins-dind running. Docker doesn’t let you start another with the same name.
```bash
docker exec -u root -it jenkins-dind bash
